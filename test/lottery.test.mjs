...
// 所以 eval 之后要让出一拍再断言。
async function run(dom, { autoPace = false } = {}) {
    // 自适应间隔默认是开的，但绝大多数用例验的是「填几秒就跑几秒」，
    // 让它自己提速会把时序断言全冲掉。要测自适应的用例显式打开。
    const key = 'hhanclub_lottery_settings_v1';
    const saved = JSON.parse(dom.window.localStorage.getItem(key) || '{}');
    saved.autoPace = autoPace;
...
/* ---------------------------------------------------------------- */
console.log('\n[51] 自适应间隔：跟着站点报的转盘时长走');
{
    // 站点每次返回 duration（转盘动画时长），页面要等它转完才让再抽。
    // 这里给 1200ms，脚本应该按 1200 + 300 缓冲排下一抽，而不是傻等填的 7 秒。
    const dom = makeDom({ pool: REAL_POOL, useBean: '每次消耗憨豆： 2000' });
    const w = dom.window;
    ...
    await run(dom, { autoPace: true });
    const d = w.document;
    check('自适应开着时，间隔输入框是灰的',
    ...
    const gaps = stamps.slice(1).map((t, i) => t - stamps[i]);
    check('间隔按 1200 + 300 排，不是填的 7 秒',
        gaps.every(gap => gap >= 1400 && gap < 2600), gaps.join(' / '));
    check('面板显示站点报的时长',
        d.getElementById('pace-info').textContent === '站点转盘 1.2s',
        d.getElementById('pace-info').textContent);
}

/* ---------------------------------------------------------------- */
console.log('\n[52] 自适应间隔：连续成功会提速，被限流就退回来并记死下限');
{
    // 不给 duration，让它从填的 1 秒起步，验的是探测逻辑本身。
    // 前 4 抽成功（第 3 抽后提速到 0.8s），第 5 抽被限流。
    const dom = makeDom({ pool: REAL_POOL, useBean: '每次消耗憨豆： 2000' });
    const w = dom.window;
    ...
    w.fetch = async url => {
        ...
        if (draws === 5) {
            return {
                ok: true, status: 200,
                text: async () => JSON.stringify({ ret: 1, msg: '请勿重复点击' })
            };
        }
        return {
            ok: true, status: 200,
    ...
    interval.disabled = false;          // 只为把起步值喂进去
    interval.value = '1';
    ...
    // 连成 3 次后应该降一档：1 秒 → 0.8 秒
    check('连成 3 次后提速到 0.8 秒',
        d.getElementById('current-interval').textContent === '0.8',
        d.getElementById('current-interval').textContent);

    const log = d.getElementById('lottery-log').textContent;
    check('提速这件事写进日志了', /提速到 0.8 秒/.test(log));
}
...
console.log('\n[53] 被限流后不再探回那个速度');
{
    ...
    // 第一抽就限流：从 1 秒起步 → 记死下限 1.2 秒 → 退到 1.5 秒
    let draws = 0;
    w.fetch = async url => {
        const target = String(url);
        if (target.includes('lucky.php')) {
        ...
        if (draws === 1) {
            return {
                ok: true, status: 200,
                text: async () => JSON.stringify({ ret: 1, msg: '操作过于频繁' })
            };
        }
        return {
            ok: true, status: 200,
    ...
    d.getElementById('max-lottery-count').value = '8';
    d.getElementById('start-lottery').click();
    await untilStopped(d, 40000);

    const shown = Number(d.getElementById('current-interval').textContent);
    check('限流后退到了 1 秒以上', shown >= 1.2, `实际 ${shown}`);
    check('后面一路成功也没再探回 1 秒以下', shown >= 1.2, `实际 ${shown}`);

    const log = d.getElementById('lottery-log').textContent;
    check('退速这件事写进日志了', /太快了，退回/.test(log), log.slice(0, 200));
}
...
console.log('\n[54] 关掉自适应，还是填几秒跑几秒');
{
    ...
    const toggle = d.getElementById('auto-pace');
    ...
    check('关掉这件事存下来了',
        JSON.parse(w.localStorage.getItem('hhanclub_lottery_settings_v1')).autoPace === false);
}
